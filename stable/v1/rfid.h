 ...
 *
*******************************************************************************/
uint8_t* data_acquisition_rfid_task() // bukan Task Free-RTOS
{
	// Inisiasi
    if (!rfid_initialized)
    {
    ...
    int trial = 0; // jumlah percobaan kirim perintah baca
    ...
    reset_iteration: // Label to jump to for continuing the outer loop
    while (trial < MAX_RFID_SCAN_TRIAL)// batas pengecekan : 20 kali, kalau gagal, kembali ke IDLE (transisi sudah ada di task lain yaitu taskOLED)
    {
//    	printf("iterasi ke-%d\r\n",trial);	// DEBUG
        ...
        cyhal_uart_clear(&uart_rfid); // Clear the UART buffer (dalam rangka gagal dapat RFID tag -> iterasi berikutnya bisa dilakukan)
		...
		while (!flag_tag_detect)
		{
			if (CY_RSLT_SUCCESS == cyhal_uart_getc(&uart_rfid, &rfid_rx_buf, 0))
			{
				...
				else
				{
					...
					// alokasikan ulang epc
					free(epc_processed);
					epc_processed = malloc(TX_BUF_SIZE * sizeof(uint8_t)); // agar mekanisme pemrosesan serupa dengan barcode
					if (epc_processed == NULL)
					{
						return NULL;
					}
					trial++;
                    goto reset_iteration; // Jump to the label to continue the outer loop
				}
			}
...
//    printf("Maximum trial reached!\r\n");		// DEBUG
	memset(epc, 0, sizeof(epc)); // kembali set 0 untuk return all zero -> nanti akan dicek di state OLED => kalau 0 semua, balik ke IDLE (ulangi)
	size_t array_length = sizeof(epc) / sizeof(epc[0]);
	hex_array_to_char_array(epc, array_length, epc_processed);
	return epc_processed;
}
