...
* Perbaikan 4.4:
* 	to-do :
* 	RFID scanning gagal -> coba lagi, limit
* 	RFID scanning data sama -> tidak simpan ke array
*
* 	done :
* 	menghidupkan kembali task buzzer
* 	setelah selesai kirim, tampilkan pada OLED.
*	pengecekan data = 0 -> tidak send
*******************************************************************************/
...
int main(void)
{
    ...
    // queue dari display task ke http_client task (array)
    ...
    // queue dari http_client task ke oled task
    	// yang menandakan apakah data sukses dikirim atau tidak
    ackQueue = xQueueCreate(1, sizeof(bool));
    if (ackQueue == NULL)
    {
        printf("Failed to create queue.\r\n");
        while(1);
...
//    modeQueue = xQueueCreate(1, sizeof(bool));
//    if (modeQueue == NULL)
//    {
//        printf("Failed to create queue.\r\n");
//        while(1);
...
void tasks_creation()
{
	...
	retval = xTaskCreate(button_task_mode, "Button Task Mode", configMINIMAL_STACK_SIZE, (void *)&taskParams, BTN_TASK_PRIORITY-4, NULL);
	if (pdPASS != retval)
	{
	...
	retval = xTaskCreate( buzzer_task, BUZZ_TASK_NAME,configMINIMAL_STACK_SIZE, (void *)&taskParams, BUZZ_TASK_PRIORITY, &buzzer_taskHandle );
	if (pdPASS != retval)
	{
