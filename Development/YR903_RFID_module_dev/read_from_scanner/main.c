...
int main(void)
{
    ...
    rslt = cyhal_uart_init(&scanner, P12_1, P12_0, NC,NC,NULL,&uart_config);

    /* Set the baud rate */
    rslt = cyhal_uart_set_baud(&uart_obj, BAUD_RATE, &actualbaud);
    rslt = cyhal_uart_set_baud(&scanner, BAUD_RATE, &actualbaud);
...
//    cyhal_system_delay_ms(UART_DELAY);
    ...
    /* Begin Rx Transfer */
    cyhal_uart_read(&scanner, (void*)rx_buf, &rx_length);
    cyhal_system_delay_ms(UART_DELAY);
