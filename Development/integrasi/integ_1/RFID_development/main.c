/*******************************************************************************
* File Name:   main.c (RFID Development)
* Description: Melakukan pengujian integrasi antara sub-sistem : Button ditekan, maka tampilan OLED akan countdown dari 10 sampai dengan 1
...
void initialize_buttons_and_interrupts()
{
	...
	/* Inisiasi RFID module : dilakukan di dalam tasknya */
//
//	/* Inisiasi Buzzer */
...
void tasks_creation()
{
	...
	retval = xTaskCreate(button_task_acq, "Button Task Acquire", BTN_TASK_STACK_SIZE, NULL, BTN_TASK_PRIORITY, NULL);
	if (pdPASS != retval)
	{
...
//	retval = xTaskCreate( red_led_task, RED_LED_TASK_NAME,LED_TASK_STACK_SIZE, NULL, LED_TASK_PRIORITY, NULL );
//
//	if( pdPASS != retval )
//	{
...
//	}
//
//	retval = xTaskCreate( green_led_task, GREEN_LED_TASK_NAME,LED_TASK_STACK_SIZE, NULL, LED_TASK_PRIORITY, NULL );
//	if (pdPASS != retval)
//	{
...
//	retval = xTaskCreate( oled_task, OLED_TASK_NAME,OLED_TASK_STACK_SIZE, NULL, OLED_TASK_PRIORITY, NULL );
//	if (pdPASS != retval)
//	{
...
//	}
	...
	retval = xTaskCreate( rfid_task, RFID_TASK_NAME,RFID_TASK_STACK_SIZE, NULL, RFID_TASK_PRIORITY, NULL );
	if (pdPASS != retval)
	{
		printf("Buzzer Task creation failed!\r\n");
	}
